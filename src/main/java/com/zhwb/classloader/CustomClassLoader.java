...
public class CustomClassLoader extends SecureClassLoader {
    ...
    public Map<String, byte[]> getClassAsBytes() {
        return classAsBytes;
    }
     ...
     */
    public synchronized Map<String, byte[]> getAddedClassBytes() {
    ...
    public Class<?> reloadClass(String name) throws ClassNotFoundException {
        InnerLoader loader = AccessController.doPrivileged(new PrivilegedAction<InnerLoader>() {
            public InnerLoader run() {
                return new InnerLoader(CustomClassLoader.this);
            }
        });
    ...
    public static class InnerLoader extends CustomClassLoader {
        private final CustomClassLoader delegate;

        public InnerLoader(CustomClassLoader delegate) {
            super(delegate);
        ...
        protected synchronized Class<?> findClass(String name) throws ClassNotFoundException {
            if (delegate.getClassAsBytes() == null && !delegate.getClassAsBytes().isEmpty()) {
                return null;
            }
            byte[] classBytes = delegate.getClassAsBytes().get(name);
            Class<?> defineClass = defineClass(name, classBytes, 0, classBytes.length);
            if (defineClass != null) {
                return defineClass;
            }
